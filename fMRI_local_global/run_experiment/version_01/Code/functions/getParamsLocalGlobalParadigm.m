...
if debug_mode
    ...
    % shown below.
    sub_code = '01';
    params.sub_code = ['subject_', sub_code];
    ...
    %======================================================================
    % GET INPUT FROM THE EXPERIMENTER & INCLUDE SANITY CHECKS
    %======================================================================
    
    sub_code = inputdlg({'Enter subject NUMBER Options: (01,02,..)'},...
        'Subject Code',1,{''});
    params.sub_code = ['subject_', sub_code{1}];
    
    
    block_type= inputdlg({'Enter block TYPE. Options: ("visual", "auditory")'},...
        'Block TYPE',1,{''});
    ...
    if not(strcmp(params.block_type,'auditory'))
        if not(strcmp(params.block_type,'visual'))
            waitfor(msgbox('Block type must be "auditory" or "visual"'))
            block_type= inputdlg({'Enter block TYPE. Options: ("visual", "auditory")'},...
                'Block TYPE',1,{''});
            params.block_type=block_type{1};
        end
    end
    block = inputdlg({'Enter run number. ( 01, 03, 05--> "visual", 02, 04, 06-->"auditory")' },...
        'Block Number',1,{''});
    params.block=str2double(block{1});
    
    
    if strcmp(params.block_type,'auditory') && (params.block==1 || params.block==3 || params.block==5)
        waitfor(msgbox('The auditory runs are only "01", "03", and "05", please select one of those!'))
        block = inputdlg({'Enter run number. ( 01, 03, 05--> "visual", 02, 04, 06-->"auditory")' },...
            'Block Number',1,{''});
        params.block=str2double(block{1});
        
    elseif strcmp(params.block_type,'visual') && (params.block==2 || params.block==4|| params.block==6)
        waitfor(msgbox('The visual runs are only "02", "04", and "06", please select one of those!'))
        block = inputdlg({'Enter run number. ( 01, 03, 05--> "visual", 02, 04, 06-->"auditory")' },...
            'Block Number',1,{''});
        params.block=str2double(block{1});
...
%% ========= PATHS =======================================================%
root=fullfile(pwd,'..');
root='/home/avalazem/Desktop/Projects/Single_Word_Processing_Stage/Long_Range_Agreement/fMRI_local_global/run_experiment/version_01';
params.path2intro_slide     = fullfile(root,'Stimuli','instructions_sentences.png');
params.path2feedback_slide = fullfile(root,'Stimuli','feedback_images');
...
% ========================================================================%
dr = dir(fullfile(params.path2stim,params.sub_code,params.block_type,['LocalGlobal' num2str(params.block) '*']));
...
params.stimulus_ontime                  = 0.5; % Duration of each word
%params.stimulus_offtime                 = 0.5; % Duration of black between words
params.SOA_visual                       = 1;
