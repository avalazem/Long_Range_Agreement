...
for trial=1:length(stimuli_words)
    ...
    base_condition=stimuli_datasets(trial,:).condition{1};
    emb=stimuli_datasets(trial,:).structure{1};
    num   =stimuli_datasets(trial,:).number{1};
    cond_event_name = join([base_condition,'_',emb,'_',num]);
    typ='number';
    %---------------------------------------------------------------------%
