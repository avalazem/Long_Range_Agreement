...
for trial=1:length(stimuli_words)
    ...
    %% ------------- UPDATE THE LOG-FILE ---------------------------------%
    % !!DANGER ZONE: Do not modify anything within this section of the code
    
    condition=stimuli_datasets(trial,:).condition{1};
    structure=stimuli_datasets(trial,:).structure{1};
    number   =stimuli_datasets(trial,:).number{1};
    cond_event_name = join([condition,'_',structure,'_',number]);
    typ='number';
    %---------------------------------------------------------------------%
    % Provide feedback to the experimenter in case multiple screens are
    % used.
%     fprintf('Block %i, trial %i, Condition %s, Emb %s, Number %s, Type %s\n', ...
%         block, trial, condition, emb, num, typ)
