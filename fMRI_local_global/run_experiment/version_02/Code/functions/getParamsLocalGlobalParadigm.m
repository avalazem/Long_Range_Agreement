...
if debug_mode
    ...
    block = inputdlg({'Enter run number. ( 01,03--> "visual", 02, 04-->"auditory")' },...
        'Block Number',1,{''});
    params.block=str2double(block{1});
    
    
    if strcmp(params.block_type,'auditory') && (params.block==1 || params.block==3)
        waitfor(msgbox('The auditory runs are only "01" and "03", please select one of those!'))
        block = inputdlg({'Enter run number. ( 01,03--> "visual", 02, 04-->"auditory")' },...
            'Block Number',1,{''});
        params.block=str2double(block{1});
        
    elseif strcmp(params.block_type,'visual') && (params.block==2 || params.block==4)
        waitfor(msgbox('The visual runs are only "02" and "04", please select one of those!'))
        block = inputdlg({'Enter run number. ( 01,03--> "visual", 02, 04-->"auditory")' },...
            'Block Number',1,{''});
        params.block=str2double(block{1});
...
%% ========= PATHS =======================================================%
root=fullfile(pwd,'..');
root='/home/cz257680/Projects/fMRI_local_global/run_experiment/version_01';
...
params.font_size    = 66; % Fontsize for words presented at the screen center
