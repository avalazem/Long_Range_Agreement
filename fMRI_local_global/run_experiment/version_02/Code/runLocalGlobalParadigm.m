%% Language-local-global paradigm. - FMRI BIMODAL VERSION
% @UNICOG,NEUROSPIN-2k20
% Christos & Yair
% ------------------------------------------------
clear; close all; clc
AssertOpenGL;
...
% randomization 
rng(int(params.run)*1e3 + int(params.subject))
% remember to export this number

% TTL settings
params.location ='Houston';  %options: 'Houston' or 'NeuroSpin', affecting hardware to use for TTLs
params.portA    = 1;
params.portB    = 0;
...
try
    ...
    % check that we get a trigger from the machine
    TTL=wait_for_first_trigger();
    if TTL ==-1
        exit(); 
    end
    %%%%%%%%%%% LOG THIS TIME HERE AS START-TIME AND SUBSTRACT IT IN THE
    %%%%%%%%%%% LOG FILES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % redefine the keys of interest
    keysOfInterest_exp=zeros(1,256);
    keysOfInterest_exp(KbName({'LeftArrow','space','RightArrow', 'ESCAPE'}))=1;
    KbQueueCreate(-1, keysOfInterest_exp);

    PsychHID('KbQueueCreate',handles.devicenum,keysOfInterest_exp);    
    PsychHID('KbQueueCreate',handles.devicenumkey,keysOfInterest_exp);    
